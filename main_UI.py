...
st.sidebar.title('Invest Wise 2023')

current_age = st.sidebar.number_input("Current Age:", value=40, min_value=0, max_value=100)
retirement_age = st.sidebar.number_input("Retirement Age:", value=50, min_value=0, max_value=100)
st.sidebar.markdown("---")
st.sidebar.markdown("#### Current Income & Assets")
average_income_mon = st.sidebar.number_input("Current Monthly Income (in ₹):", value=202000, min_value=0, max_value=10000000000)
term_ins_cover = st.sidebar.number_input("Current Term Insurance Cover (in ₹):", value=0, min_value=0, max_value=10000000000)
health_ins_cover = st.sidebar.number_input("Current Health Insurance Cover (in ₹):", value=600000, min_value=0, max_value=10000000000)
emergency_fund = st.sidebar.number_input("Emergency Fund Saved (in ₹) for unforeseen circumstances: ", value=10000, min_value=0, max_value=10000000000)
curr_asset = st.sidebar.number_input("Value of Non-Movable Asset (in ₹) when Converted ", value= 20000000, min_value=0, max_value=10000000000)
st.sidebar.markdown("---")
st.sidebar.markdown("#### Current Liabilities and Expenses")
monthly_emi = st.sidebar.number_input("Current EMI(in ₹)", value=100000, min_value=0, max_value=10000000000)
...
rental_income_ret = st.sidebar.number_input("Post-Retirement Rental Income (in ₹)", value=45000, min_value=0, max_value=10000000000)
...
if st.sidebar.button('Calculate Retirement Readiness'):
    term_suggestion, health_suggestion, emergency_suggestion, surplus_suggestion, \
        other_suggestion, readiness, dilution_suggestion, readiness_suggestion = mn.start_readiness(\
    ...
    st.markdown("### Retirement Preparedness Level")
    ...
    bullet_points = [term_suggestion, health_suggestion, emergency_suggestion, surplus_suggestion, other_suggestion, dilution_suggestion, readiness_suggestion]
